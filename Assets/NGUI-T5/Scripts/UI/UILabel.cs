...
public class UILabel : UIWidget
{
	...
	public bool enableColorCode = false;
	public bool ignoreArabic = false;
	...
	[HideInInspector][SerializeField] Vector2 mShadowDistance = Vector2.one;
	...
	public string text
	{
		...
		set
		{
			...
			else if (mText != value)
			{
				...
				//if (NGUIUtils.ContainArabicCharacters(value) && !ignoreArabic) {
				//	if (value.Contains("[-]")) {
				//		//Debug.Log("@@@@@@0: "+value);
				//		//Debug.Log("TEST: "+"test".Insert(0,"DM"));
				//		string pattern = @"\[[a-zA-Z0-9]+\]";
				//		MatchCollection match = Regex.Matches(value, pattern);
				//		string pattern2 = @"\](.*)\[";
				//		MatchCollection match2 = Regex.Matches(value, pattern);
				//		value = value.Replace("[-]", "");
				//		value = Regex.Replace(value, pattern, "");
				//		//	Debug.Log("@@@@@@1: "+value);
				//		value = NGUIUtils.ReverseWords(ArabicSupport.ArabicFixer.Fix(NGUIUtils.RevertSpecialWordArabic(value)));
				//		//Debug.Log("@@@@@@2: "+value);
				//		int countMatch = match.Count - 1;
				//		for (int i = 0; i < match.Count; i++)
				//		{
				//			//Debug.Log("#######: "+match[i].Value+"///"+ match[i].Index+"///"+ match2[i].Index);
				//			value = value.Insert(match[i].Index, match[countMatch - i].Value);
				//			if (i < match.Count - 1) {
				//				value = value.Insert(match[i + 1].Index - 3, "[-]");
				//			}
				//			//value.Insert(match[i].Index,match[i].Value);
				//			//Debug.Log("%%%%%%%%%: "+value);
	...
	/// <summary>
	/// Shadow distance in pixels : Outline and Shadow.
	/// </summary>
		...
		if (mFinalFontSize > 0)
		{
			...
			for (int ps = mFinalFontSize; ps > 0; --ps)
			{
				...
				bool fits = NGUIText.WrapText(printedText, out mProcessedText, true, false,
				                              mOverflowEllipsis && mOverflow == Overflow.ClampContent, ignoreArabic);
	...
	public override void OnFill (BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color> cols)
	{
		...
		if (effectStyle != Effect.None)
		{
			...
			if (effectStyle != Effect.OutlineAndShadow && effectStyle != Effect.Outline8AndShadow) {
				ApplyShadow(verts, uvs, cols, offset, end, pos.x, -pos.y);
			} else {
				ApplyShadow(verts, uvs, cols, offset, end, pos.x * mShadowDistance.x, -pos.y * mShadowDistance.y);
			}


			if ((effectStyle == Effect.Outline) || (effectStyle == Effect.Outline8 || effectStyle == Effect.OutlineAndShadow || effectStyle == Effect.Outline8AndShadow))
			{
				offset = end;
				end = verts.size;
