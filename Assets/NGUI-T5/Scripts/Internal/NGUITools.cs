...
//    }
	...
	static public Camera FindCameraForLayer (int layer, string cameraTag)
	{
		int layerMask = 1 << layer;
		Camera cam;
		...
		{
			cam = UICamera.list.buffer[i].cachedCamera;
			if (cam && (cam.cullingMask & layerMask) != 0 && (cameraTag == null || cam.tag == cameraTag))
				return cam;
		}

		cam = Camera.main;
		if (cam && (cam.cullingMask & layerMask) != 0 && (cameraTag == null || cam.tag == cameraTag)) return cam;
		...
		{
			cam = cameras[i];
			if (cam && cam.enabled && (cam.cullingMask & layerMask) != 0 && (cameraTag == null || cam.tag == cameraTag))
				return cam;
		}
		return null;
	}
	...
	static public GameObject AddChildNoInstantiate (this GameObject parent, GameObject go)       {
// #if UNITY_EDITOR
//             UnityEditor.Undo.RegisterCreatedObjectUndo(go, "Create Object");
// #endif
	if (go != null && parent != null)
