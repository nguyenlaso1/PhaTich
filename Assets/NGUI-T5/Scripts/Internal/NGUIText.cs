...
static public class NGUIText
{
	...
	static public bool WrapText (string text, out string finalText, bool keepCharCount, bool wrapLineColors, bool useEllipsis = false, bool ignoreArabic = false)
	{
		...
		if (!ignoreArabic && NGUIUtils.ContainArabicCharacters(finalText)) {
			string result = "";
			string[] lineString = finalText.Split('\n');
			for (int i = 0; i < lineString.Length; i++) {
				result += NGUIUtils.ReverseWords(lineString [i]);                               
