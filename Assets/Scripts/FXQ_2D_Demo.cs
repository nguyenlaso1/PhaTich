...
public class FXQ_2D_Demo : MonoBehaviour
{
	...
	{
		GUIAnimSystemFREE.Instance.SetGraphicRaycasterEnable(this.m_2DDemo_UI, false);
		...
		GUIAnimSystemFREE.Instance.SetGraphicRaycasterEnable(this.m_2DDemo_UI, true);
		this.ShowParticle();
		this.SetParticleType(0);
		this.UpdateToggleSpriteAndDarkScreen();
		yield break;
	}
		...
		if (this.m_ParticleIndex != this.m_ParticleIndexOld || this.m_ParticleType != this.m_ParticleTypeOld)
		{
			...
			foreach (object obj3 in this.m_ParticleTypeList[this.m_ParticleType].m_Particles)
			{
				...
				if (num == this.m_ParticleIndex)
				{
					...
					if (this.m_ParticleOld != null)
					{
						this.m_TargetAnimatorEvent_Old = this.m_ParticleOld.gameObject.GetComponent<FXQ_TargetAnimatorEvent>();
					}
					...
					if (this.m_Particle != null)
					{
						...
						this.m_TargetAnimatorEvent_Current = this.m_Particle.gameObject.GetComponent<FXQ_TargetAnimatorEvent>();
						this.UpdateTargetAnimator();
					}
					break;
				}
				num++;
	...
	private void UpdateTargetAnimator()
	{
		if (this.m_TargetAnimatorEvent_Old != null && this.m_TargetAnimatorEvent_Old != this.m_TargetAnimatorEvent_Current)
		{
			this.m_TargetAnimatorEvent_Old.m_TargetAnimator.gameObject.SetActive(false);
		}
		if (this.m_TargetAnimatorEvent_Current == null)
		{
			return;
		}
		if (!this.m_TargetAnimatorEvent_Current.m_TargetAnimator.gameObject.activeSelf && this.m_Options_Toggle_ShowSprite.isOn)
		{
			this.m_TargetAnimatorEvent_Current.m_TargetAnimator.gameObject.SetActive(true);
		}
		if (this.m_TargetAnimatorEvent_Current.m_ParticleEvent != ParticleEvent.None)
		{
			if (this.m_TargetAnimatorEvent_Current.m_ParticleEvent == ParticleEvent.Attack)
			{
				if (this.m_TargetAnimatorEvent_Current.m_TargetAnimator.gameObject != null && this.m_TargetAnimatorEvent_Current.m_TargetAnimator.gameObject.activeSelf)
				{
					this.m_TargetAnimatorEvent_Current.m_TargetAnimator.gameObject.SendMessage("UnderAttack", SendMessageOptions.DontRequireReceiver);
				}
			}
			else if (this.m_TargetAnimatorEvent_Current.m_ParticleEvent == ParticleEvent.UI)
			{
			}
		}
	}

	private void UpdateToggleSpriteAndDarkScreen()
	{
		if (this.m_Options_Toggle_ShowSprite.isOn)
		{
			foreach (GameObject gameObject in this.m_SpriteToToggle)
			{
				if (gameObject == this.m_TargetAnimatorEvent_Current.m_TargetAnimator)
				{
					gameObject.SetActive(this.m_Options_Toggle_ShowSprite.isOn);
				}
				else
				{
		...
		{
			foreach (GameObject gameObject2 in this.m_SpriteToToggle)
			{
				gameObject2.SetActive(false);
			}
		}
		foreach (GameObject gameObject3 in this.m_DarkScreenToToggle)
		{
			if (gameObject3.activeSelf != this.m_Options_Toggle_DarkScreen.isOn)
			{
				gameObject3.SetActive(this.m_Options_Toggle_DarkScreen.isOn);
			}
		}
	}
	...
	{
		GUIAnimSystemFREE.Instance.LoadLevel("3D Demo", 1f);
	...
	{
		this.SetParticleType(6);
		FXQ_SoundController.Instance.Play_SoundClick();
	}

	public void Button_ParticleType_UI_Back()
	{
		this.SetParticleType(7);
		FXQ_SoundController.Instance.Play_SoundClick();
	}
	...
	public Toggle m_Options_Toggle_ShowSprite;

	public Toggle m_Options_Toggle_DarkScreen;
	...
	public FXQ_TargetAnimatorEvent m_TargetAnimatorEvent_Current;

	public FXQ_TargetAnimatorEvent m_TargetAnimatorEvent_Old;

	public GameObject[] m_SpriteToToggle;

	public GameObject[] m_DarkScreenToToggle;
}
