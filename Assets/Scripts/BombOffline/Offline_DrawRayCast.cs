// @sonhg: class: BombOffline.Offline_DrawRayCast
using System;
using UnityEngine;

namespace BombOffline
{
	public class Offline_DrawRayCast : MonoBehaviour
	{
		private void FixedUpdate()
		{
			if (this.actor.CurrentDirection == this.direction || this.actor.enableAllRayCast)
			{
				this.CheckCollision();
				if (this.draw)
				{
		...
		public bool CheckCollision()
		{
			...
			if (this.actor.CanMoveThrough)
			{
				layerMask = 1 << LayerMask.NameToLayer("BorderWall");
			}
		...
		public Offline_BaseCharactersController actor;
